...
function ConversorDeMoedas() {
    ...
    const [deMoeda, setDeMoedas] = useState('USD');
    const [paraMoeda, setParaMoeda] = useState('EUR');
    const [quantidade, setQuantidade] = useState(1);
    const [resultado, setResultado] = useState(0);

    useEffect(() => {
        fetch('https://api.exchageratesapi.io/latest')
    ...
    const converterMoedas = () => {
        fetch('https://api.exchageratesapi.io/latest?base=${deMoeda}&symbols=${paraMoeda}')
            .then(response => response.json())
            .then(data => {
